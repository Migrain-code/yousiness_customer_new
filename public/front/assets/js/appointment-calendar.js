(function($) {
    ...
    $( document ).ready(function() {
        ...
        function c(passed_month, passed_year, calNum) {
            ...
            calendar.datesBody.find('div').each(function(index) {
                var date = parseInt($(this).text());
                var weekday = getWeekdayNum(passed_year, passed_month, date);
                if (tatilGunleri.includes(weekday)) {
                    $(this).addClass('past-date disabled-passed'); // veya başka bir stil ile devre dışı bırakabilirsiniz
            ...
            if (!$.isEmptyObject(selected)) {
                ...
                function highlightDates(passed_year, passed_month, dateElements) {
                    if (passed_year in selected && passed_month in selected[passed_year]) {
                        ...
                        if (daysToCompare.length > 0) {
                            // Sadece ilk tarihi seç
                            var firstDay = daysToCompare[daysToCompare.length - 1];
                            dateElements.each(function(index) {
                                if (parseInt($(this).text()) === firstDay) {
